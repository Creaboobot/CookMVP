...
The browser offers a single natural-language `Ingredients and craving` field for typed, pasted, and spoken requests. Where `MediaRecorder` and microphone permission are available, Cookooi records a short in-app audio note, uploads it to `/api/voice/transcribe`, places the returned transcript in that same field, and then runs the same parser used by typed requests. The MVP parser converts the field into the request fields above and sends only the structured request payload to `/api/recipes/generate`.
...
The browser uploads one recorded audio blob as `audio`; `file` is accepted as a compatibility field. The browser must never call OpenAI directly and must never receive an API key. The endpoint is independent from recipe generation so the user can edit the transcript in the combined request field if generation cannot continue safely.
...
The transcript is returned to the browser field for the current request. It should not be written to feedback/session analytics by default.
...
- Do not clear typed or pasted request text when recording or transcription fails.
- Keep the editable combined request field available for unsupported browsers.
