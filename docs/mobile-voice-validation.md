...
- Confirmed the public root returned `200 OK` and served the Task 26 recorder UI with `Record voice note`.
...
- Confirmed `POST /api/recipes/generate` on the public route returned exactly three OpenAI-backed recipe proposals for the parsed transcript fields.
- Confirmed an automated 390px mobile Safari-profile browser smoke on the public HTTPS route:
  ...
  - the button moved from `Record voice note` to recording state and back
  - transcription populated the editable transcript
  - parsed review fields included available items, craving, avoidances, dinner, three servings, 30 minutes, and stovetop
...
6. Review and edit the transcript and parsed fields.
7. Generate meal ideas and verify three proposal cards appear.

If microphone access is denied or recording is unavailable, paste a transcript into the voice note field and use `Parse note`.
